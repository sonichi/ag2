...
- To build the developer Docker image (`ag2labs_dev_img`), use the following commands:
  ...
  ```bash
  docker build -f .devcontainer/dev/Dockerfile -t ag2labs_dev_img https://github.com/ag2labs/autogen.git#main
  ```
...
- For building the developer image built from a specific Dockerfile in a branch other than main/master
  ...
  git clone --branch {branch-name} https://github.com/ag2labs/autogen.git
...
We have provided the configuration in [devcontainer](https://github.com/ag2labs/autogen/blob/main/.devcontainer). They can be used in GitHub codespace too. Developing AutoGen in dev containers is recommended.
